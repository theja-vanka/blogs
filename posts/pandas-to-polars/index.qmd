...
```
```{python}
polars_df.groupby('birds').agg(pl.col('age').mean())
```
...
```{python}
pandas_df = pandas_df.append(pd.DataFrame(new_data, index=['k']))
pandas_df
```
### Delete new row from pandas dataframe
```{python}
pandas_df = pandas_df.drop('k')
pandas_df
```
### Append new row to polars dataframe
```{python}
new_data['index'] = 'k'
polars_df = polars_df.append(pl.DataFrame(new_data))
polars_df
```
### Delete new row from polars dataframe
```{python}
